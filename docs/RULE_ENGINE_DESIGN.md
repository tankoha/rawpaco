...
このドキュメントは rawpaco の lint ルールエンジン全体の設計をまとめたものです。CLAUDE.md の開発ルール、README.md の目的・実装方針を前提とし、これらと矛盾しないように設計しています。決着していない大きな方針転換候補は「8. 未決事項・提案」に切り出し、断定を避けています。全体のシステムフローをMermaid図で俯瞰したい場合は [docs/SYSTEM_FLOW.md](SYSTEM_FLOW.md) を参照してください。
...
- **重要度別（warning/error）の終了コード制御**: 4節では単純化のため「診断1件でも終了コード1」としましたが、運用が進みルール数が増えた段階で「このルールは注意喚起のみでCIは落とさない」という段階的な重要度制御が欲しくなる可能性があります。現時点ではルールが0個のため必要性を判断できず、导入を見送っています。
