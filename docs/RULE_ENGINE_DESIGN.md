...
このドキュメントは rawpaco の lint ルールエンジン全体の設計をまとめたものです。CLAUDE.md の開発ルール、README.md の目的・実装方針を前提とし、これらと矛盾しないように設計しています。決着していない大きな方針転換候補は「8. 未決事項・提案」に切り出し、断定を避けています。
