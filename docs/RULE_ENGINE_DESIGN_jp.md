...
- [0. 前提・制約](#0-前提制約)
- [1. ルールエンジンAPI設計](#1-ルールエンジンapi設計)
  - [1.1 全体アーキテクチャ](#11-全体アーキテクチャ)
  - [1.2 TSBindings.pas の拡張が必要](#12-tsbindingspas-の拡張が必要)
  - [1.3 走査ドライバ (ASTWalker.pas)](#13-走査ドライバ-astwalkerpas)
  - [1.4 ルールの実装単位とインターフェース](#14-ルールの実装単位とインターフェース)
  - [1.5 診断とコンテキスト](#15-診断とコンテキスト)
  - [1.6 実行フロー（擬似コード）](#16-実行フロー擬似コード)
- [2. 5つの問題点ごとの検知方針](#2-5つの問題点ごとの検知方針)
  - [2.1 一貫性のムラ（命名規則・エラーハンドリングの不統一）](#21-一貫性のムラ命名規則エラーハンドリングの不統一)
  - [2.2 過剰な防御的コーディング](#22-過剰な防御的コーディング)
  - [2.3 依存関係の楽観性（hallucination）](#23-依存関係の楽観性hallucination)
...
- [5. 自己lint組み込みへの道筋（鶏卵問題）](#5-自己lint組み込みへの道筋鶏卵問題)
- [6. 実装優先順位と最初の数個のルール具体案](#6-実装優先順位と最初の数個のルール具体案)
  - [P1: RAWPACO-DEFENSE-001 空のexceptハンドラ（例外の握りつぶし）](#p1-rawpaco-defense-001-空のexceptハンドラ例外の握りつぶし)
  - [P2: RAWPACO-SEC-001 SQL文字列連結パターン](#p2-rawpaco-sec-001-sql文字列連結パターン)
  - [P3: RAWPACO-SEC-002 シークレットらしき文字列のハードコード](#p3-rawpaco-sec-002-シークレットらしき文字列のハードコード)
  - [P4: RAWPACO-DEPR-001 自己矛盾する非推奨API使用（同一ファイル内）](#p4-rawpaco-depr-001-自己矛盾する非推奨api使用同一ファイル内)
  - [P5: RAWPACO-STYLE-001 命名規則チェック（設定ファイルベース）](#p5-rawpaco-style-001-命名規則チェック設定ファイルベース)
  - [P6: RAWPACO-DEPR-002 FPC RTL/FCLの`deprecated`シンボル一覧に基づく検知](#p6-rawpaco-depr-002-fpc-rtlfclのdeprecatedシンボル一覧に基づく検知)
...
*(注: 日本語見出しのアンカーはMarkdownレンダラごとに生成規則が異なるため、GitHub上での実際のリンク先とずれる場合がある。正確なアンカーは英語版 [docs/RULE_ENGINE_DESIGN.md](RULE_ENGINE_DESIGN.md) を参照。)*
