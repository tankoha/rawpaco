...
*This document is a Japanese translation kept for reference. The canonical, up-to-date version is [docs/flows/rawpaco_lpr.md](rawpaco_lpr.md) (English).*

現状の `rawpaco.lpr` は本格的なlint実行ではなく、tree-sitter-pascalが正しくリンク・動作することを確認する自己チェックプログラムです（[docs/RULE_ENGINE_DESIGN_jp.md](../RULE_ENGINE_DESIGN_jp.md) 1.1節で述べている通り、ルールエンジン実装時にこの内容は `LintDriver` 等へ置き換わる想定）。
