...
- Windows CI 6回目実行（run 30761956791）: `Invoke-WebRequest` でのダウンロードが実行可能ファイルとして壊れており（SourceForgeのミラー選択リダイレクトを正しく辿れなかったと推測）、`Start-Process`が「corrupted and unreadable」で失敗。ローカルでの`curl -sL`検証では正常にリダイレクトを辿れていたため、ダウンロード手段を`curl.exe`（Windowsランナー標準搭載）に切替え。未検証・要フォローアップ。
