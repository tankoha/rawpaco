...
| RAWPACO-SEC-002 | シークレットらしき文字列のハードコード検知 | 実装済み | `src/Rules/RuleSec002.pas`。設計書P3。positive 7件/negative 5件。`identifier := literalString`の単純代入・`Obj.Password := 'x'`のような`exprDot`経由のプロパティ代入(ネスト可、2026-08-04追加)・`declVar`/`declConst`の初期値付き宣言が対象。識別子名は接尾辞一致(単純部分一致だと`TokenList`等を誤検知するため)、値はプレースホルダらしき部分一致で除外 |
...
- **RAWPACO-SEC-002のexprDot対応(2026-08-04、Fable5のレビュー advisoryを受けて対応)**: `Obj.Password := 'x'`のような`exprDot`経由のプロパティ・フィールド代入も検知対象に追加した。ネストした`A.B.Password`のような形でも、`exprDot`は左結合で外側の`rhs`フィールドが常に末尾の識別子になるため(実機プローブで確認済み)、再帰なしで`rhs`フィールドを見るだけで済んだ。接尾辞一致による判定は末尾の識別子のみに対して行うため、`Edit1.PasswordChar := '*'`(LazarusのTCustomEdit系が持つマスク表示用プロパティ)のような、`Password`を含むが無関係な識別子を自然に誤検知しない。fpc-source全体(5395ファイル)で実測したところ新規誤検知はゼロで、新たに検知した2件(`packages/fcl-db/tests/tcgensql.pas`の`D.Password:='Secret'`/`C.Password:='p'`)はいずれもテストコード内のフィクスチャ値だが、パターンとしては設計通りの真陽性(意味は分からないので当然フラグする)。`varAssignDef`(inline var代入のlhs)は引き続き対象外(出現頻度が低く対応コストに見合わないため)。詳細は`src/Rules/RuleSec002.pas`冒頭コメント参照。
